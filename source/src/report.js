...
(function () {
  ...
  E.api({ id: id }, 'GET').then(function (r) {
    ...
    el.innerHTML = '<p class="eyebrow">Shared analysis · ' + esc(OBJ[a.objective] || a.objective) + ' · ' + esc(String(a.industry).replace('_', ' ')) + ' · ~' + esc(a.size) + ' people</p>' +
      ...
      '<p class="notice">Generated ' + new Date(r.created_at).toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: 'numeric' }) + '. Current conditions updated 24 Sep 2026; structural scores March 2026. Costs are indicative model estimates. Decision support only — not legal, tax or investment advice.</p>';
